...
| **Evidence Source** | **(a) Change control — MECHANISM RESIDENT, APPLICABILITY UNRESOLVED.** A resident definition exists: `D8 §70 — Architecture Change Control` (PD-02 corpus, FROZEN) states the six required elements of an architecture change; `C2 §32` and `E6 §48` route into it; `D8 §23` adds a Baseline Update Control Point. At constitutional tier, `Engineering Constitution §3.4` makes an **ADR** *"the required instrument for any architectural-tier change"*, and Architecture Freeze `:14` routes architectural change to Constitution §3. **What remains unresolved is which of these applies to a Volume-level change:** `GDR-0017` requires *"the **applicable** Architecture Change Control"* and names none. `GDR-0026 §1` classifies **Volume-level *lifecycle state*** as **Founder-reserved, not Architectural Tier**, with this Register as the canonical vehicle — which settles lifecycle transitions but not a post-activation *architectural content* change. Established under `ACT-CC-R13B` / `ACT-CC-R14C` (disposition **B**) and `ACT-CC-R14X` Workstream C (**AUTHORITY REQUIRED**). **(b) Revocation — RESOLVED.** No resident revocation mechanism exists, and the Founder has **expressly determined that irreversibility is intended** (`GDR-0030`, `DEC-REVOCATION` = irreversible by design), which is the Pass Condition's second disjunct. *(Historical: this field previously read "EVIDENCE NOT ESTABLISHED, both halves"; corrected under `ACT-CC-R14X` Workstream A.)* |
