...
## 13. Future Evolution
[O] Roles, workforce, budgets, KPIs and Department lifecycle *states* are **not** in Freeze §4 and are reserved to the Architect. **Agent Definition ownership (INV-2 clause 1) — RESOLVED, both sides.** The Department side was built under `ACT-CC-F03-038` as `Department.owned_agent_definitions`; the Agent Definition side was built under `ACT-CC-F03-039` (`DEC-AGENT-DEPT-OWNERSHIP = OPTION A`) as `AgentDefinition.owning_department_key`, and the two are reconciled by `resolve_agent_definitions` / `disputed_agent_definition_ownership` / `unresolved_agent_definition_ownership` / `unbacked_agent_definition_claims`. Both sides hold plain keys, so §8's prohibition on depending on Agent is preserved and the Agent boundary takes no `capability` import either; a caller that can see both supplies the pairs.

**Correction of provenance.** `ACT-CC-F03-038` recorded the Agent Definition side as `[O]` reserved under `agent_spec §12`/`§13`. **That was wrong**, and the error was this program's own. Ownership is `[E]` ratified in three canonical sources and reserved in none: `agent_spec §3 Owned Data` — *"Agent Definitions owned by exactly one Department (INV-2)"*; Freeze §4's Agent Definition entry — *"Ownership: exactly one Department"*; Domain Model §5 — `Agent Definition → Exactly one Platform Division`. The `agent_spec §12`/`§13` reservation covers Agent **construction** discipline — *"governed construction of Definitions/Instances"* — not ownership. The field had been absent for a historical reason instead: in Phase 3 the Department entity did not exist, only the `DepartmentRef` stub.

**INV-2 clause 2 — RESOLVED elsewhere.** That a Definition *implements at least one Capability* was realized under `ACT-CC-F03-040` (`DEC-AGENT-FACTORY-INV2-CLAUSE2 = OPTION A`) on the Agent Definition contract and, for the corpus view, on `CapabilityGraph`. It is **not** a Department fact and is neither enforced nor assumed by this boundary — a Department owns Agent Definitions; it does not evaluate what they implement. [O] Governed **creation, registration and lifecycle** of Agent Definitions remain reserved to the Architect (`agent_spec §12`/`§13`, Agent Factory).
